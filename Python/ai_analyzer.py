...
"""
    ...
    def _parse_scoring_response(self, response_text):
        ...
        try:
            ...
            # Check if response is already a dictionary/JSON object
            ...
            try:
                ...
                logger.info("Response is not JSON, parsing as text")
                pass
            ...
            for section in sections:
                ...
                if section.startswith('[Scoring]:'):
                    ...
                    lines = section.split('\n')[1:]  # Skip the [Scoring]: line
                    
                    for line in lines:
                        if ':' in line and 'x' in line and '=' in line:
                            # Parse format: [Parameter Name]: [Score] x [Weightage]% = [Rating]
                            try:
                                parts = line.split(':')
                                if len(parts) >= 2:
                                    param_name = parts[0].strip().replace('[', '').replace(']', '')
                                    score_part = parts[1].strip()
                                    
                                    # Extract score, weightage, and rating
                                    if 'x' in score_part and '=' in score_part:
                                        score_weight = score_part.split('x')[0].strip()
                                        weight_rating = score_part.split('x')[1].strip()
                                        
                                        if '%' in weight_rating and '=' in weight_rating:
                                            weight = weight_rating.split('%')[0].strip()
                                            rating = weight_rating.split('=')[1].strip()
                ...
                elif section.startswith('[Final_Match]:'):
                    # Parse final score
                    try:
                        score_line = section.split('\n')[1] if len(section.split('\n')) > 1 else ''
                        
                        # Try to extract numeric value from the line
                        import re
                        numbers = re.findall(r'\d+\.?\d*', score_line)
                        if numbers:
                            # Find the first number that could be a score (1-10 range)
                            for num in numbers:
                                score = float(num)
                                if 1 <= score <= 10:
                                    result['final_score'] = score
                                    logger.info(f"Parsed final score from [Final_Match]: {score}")
                                    break
                            else:
                                # If no valid score found in this section, continue
                                logger.warning(f"No valid score found in [Final_Match] section: {score_line}")
                        elif score_line.strip().replace('.', '').isdigit():
                            score = float(score_line.strip())
                            if 1 <= score <= 10:
                                result['final_score'] = score
                                logger.info(f"Parsed final score from [Final_Match]: {score}")
                    except Exception as e:
                        logger.warning(f"Could not parse final score from [Final_Match]: {e}")
                        # Don't set default here, let the fallback logic handle it
                
                elif section.startswith('[Consideration]:'):
                    # Parse consideration
                    consideration_lines = section.split('\n')[1:]  # Skip the [Consideration]: line
                    result['consideration'] = '\n'.join(consideration_lines).strip()
                
                elif section.startswith('[Recommendation]:'):
                    # Parse recommendation
                    recommendation_lines = section.split('\n')[1:]  # Skip the [Recommendation]: line
                    recommendation_text = '\n'.join(recommendation_lines).strip()
                    
                    # Map recommendation to standard categories
                    if 'interview' in recommendation_text.lower():
                        result['recommendation'] = 'To be interviewed'
                    elif 'reject' in recommendation_text.lower():
                        result['recommendation'] = 'Candidature rejected'
                    else:
                        result['recommendation'] = 'Review further'
                
                else:
                    # If we don't recognize the section, try to extract any numeric score
                    import re
                    numbers = re.findall(r'\d+\.?\d*', section)
                    if numbers and result['final_score'] == 0:
                        # Find the first number that could be a score (1-10 range)
                        try:
                            for num in numbers:
                                score = float(num)
                                if 1 <= score <= 10:
                                    result['final_score'] = score
                                    logger.info(f"Extracted score from unrecognized section: {score}")
                                    break
                        except:
                            pass
            
            # Ensure all required fields are present and validate final score
            if 'final_score' not in result or result['final_score'] == 0:
                # Try to extract any numeric score from the response text
                import re
                numbers = re.findall(r'\d+\.?\d*', response_text)
                if numbers:
                    # Find a reasonable score in 1-10 range
                    for num in numbers:
                        try:
                            score = float(num)
                            if 1 <= score <= 10:
                                result['final_score'] = score
                                logger.info(f"Fallback final score extraction: {score}")
                                break
                        except:
                            continue
                
                # If still no score found, calculate from parameter scores
                if result['final_score'] == 0 and result['parameter_scores']:
                    total_rating = sum(param.get('rating', 0) for param in result['parameter_scores'].values())
                    result['final_score'] = min(10, max(1, total_rating))
                    logger.info(f"Calculated final score from parameters: {result['final_score']}")
                elif result['final_score'] == 0:
                    # Last resort fallback
                    result['final_score'] = 5.0
                    logger.warning("Using default final score: 5.0")
            
            # Ensure final score is within valid range
            result['final_score'] = max(1, min(10, float(result['final_score'])))
            
            logger.info(f"Parsing completed. Final score: {result['final_score']}")
            return result
            ...
            return {
                ...
                'final_score': 5.0,
                'recommendation': 'Review further',
                'consideration': 'Unable to parse detailed scoring analysis.'
            }
