# Database Migration Guide for Plan-based CV Processing Limits

This guide explains how to implement the plan-based CV processing limits and payment tracking system.
...
## 2. Set up Monthly Reset Cron Job

Create a cron job or scheduled function to reset CV counts monthly:

```sql
-- This should be run monthly (e.g., on the 1st of each month)
SELECT reset_monthly_cv_count();
```

## 3. Test the Implementation

1. **Test CV Processing Limits:**
   - Create a test company with a plan that has limited CVs
   - Try to process CVs up to the limit
   - Verify that the system blocks processing when the limit is reached

2. **Test Payment Recording:**
   - Make a test payment through the recharge button
   - Verify that the payment is recorded in the database
   - Check that the company's subscription is updated

3. **Test Usage Tracking:**
   - Process some CVs and check the usage tracking table
   - Verify that the CV count is incremented correctly

## 4. Monitoring and Maintenance

1. **Monitor Usage:**
   - Regularly check the `company_usage_tracking` table for usage patterns
   - Monitor the `subscription_payments` table for payment issues

2. **Monthly Reset:**
   - Ensure the monthly reset function runs on schedule
   - Monitor for any companies that might need manual intervention

3. **Plan Updates:**
   - When updating plans, ensure the `max_cvs` field is properly set
   - Test the system with different plan configurations
...
2. **Caching:**
   - Consider caching frequently accessed plan information
...
1. **CV Count Not Incrementing:**
   - Check if the `increment_cv_count` function is being called
   - Verify that the company has a valid plan

2. **Payment Not Recorded:**
   - Check Razorpay webhook configuration
   - Verify database permissions for the payment recording function

3. **RLS Policy Issues:**
   - Ensure users have proper company associations
   - Check that the auth.uid() is properly set

### Debug Queries:

```sql
-- Check company usage
SELECT c.company_name, c.cv_processed_count, p.plan_name, p.max_cvs
FROM companies c
LEFT JOIN plans p ON p.plan_name = c.selected_plan;

-- Check recent usage tracking
SELECT * FROM company_usage_tracking 
...
JOIN plans p ON p.plan_id = sp.plan_id
